...
def main(save_to, num_epochs, feature_maps=None, mlp_hiddens=None,
    ...
    #         .copy(name='cost'))
    # error_rate = (MisclassificationRate().apply(y.flatten(), probs.flatten())
    #               .copy(name='error_rate'))

    EPS = 1e-9
    MONITORING_BATCH_SIZE = 128
    BATCH_SIZE = 128
    NCLASSES = output_size
    cost = -(tensor.sum(y * tensor.log(probs + EPS)) + tensor.sum((1 - y) * tensor.log(1 - probs + EPS)))
    error_rate = tensor.sum(tensor.abs_(y - probs)) / BATCH_SIZE / NCLASSES
    cost.name = 'cost'
    ...
    step_rule_c = Momentum(learning_rate=1e-4)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--feature-maps", type=int, nargs='+',
                        default=[16, 32, 64], help="List of feature maps numbers.")
    parser.add_argument("--mlp-hiddens", type=int, nargs='+', default=[200],
